...
def main():
    ...
    qpband = loadmat('qpbandg.mat',mat_dtype=True)
    qp_ek = qpband["ek"]
    Rho =qpband["rho"]
    ...
    # BC_d = loadmat('BC_d.mat',mat_dtype=True)
    # bc_d = BC_d['bc']
    ...
    colors2 = ["C2","#D7C8DE","C3"]
    custom_cmap2 = LinearSegmentedColormap.from_list("CustomMap2", colors2, N=255)

    cmapline= plt.get_cmap(custom_cmap)
    norm = plt.Normalize(-1, 1)
    line_colors1_mu05 = cmapline(norm(Rho[:,0]))
    line_colors2_mu05 = cmapline(norm(Rho[:,1]))
    line_colors3_mu05 = cmapline(norm(Rho[:,2]))
    line_colors4_mu05 = cmapline(norm(Rho[:,3]))
    ...
    plt.subplots_adjust(top=0.98,bottom=0.08,left=0.08,right=0.98,hspace=0.0,wspace=0.25)
    ...
    axa = axes[0]
    # band spectrum
    axa.plot(k,elec_Ek1,linewidth=lw_band, color='#008fd5')
    axa.plot(k,elec_Ek2,linewidth=lw_band, color='#008fd5')
    # chemcal potential
    axa.plot(muline_b,linewidth=lw_mu,linestyle='--',color='k')
    axa.plot(muline_c,linewidth=lw_mu,linestyle='--',color='k')
    axa.plot(muline_d,linewidth=lw_mu,linestyle='--',color='C1')
    axa.plot(muline_e,linewidth=lw_mu,linestyle='--',color='k')
    axa.set_box_aspect(1)
    axa.set_ylim([-3,3])
    axa.set_yticks([-2,-1,0,1,2])
    axa.set_ylabel(r'$\epsilon_k/(2t)$',fontsize = legftsz, labelpad=-5)
    axa.set_xticks(tick_label)
    axa.set_xticklabels(name_label,fontsize = legftsz)
    # axa.set_xlabel('high symmetry points')
    axa.text(1.01, 0.11, r'a', transform=axa.transAxes, fontsize=ftsz, va='top')
    axa.text(1.01, 0.2, r'b', transform=axa.transAxes, fontsize=ftsz, va='top')
    axa.text(1.01, 0.28, r'c', transform=axa.transAxes, fontsize=ftsz, va='top')
    axa.text(1.01, 0.53, r'd', transform=axa.transAxes, fontsize=ftsz, va='top')
    axa.text(x_labeltext, y_labeltext, '(a)', transform=axa.transAxes, fontsize=ftsz, va='top')


    axb = axes[1]
    #axb.plot(k,Ek[:,0],linewidth=linewidth1,color=line_colors1)
    #axb.plot(k,Ek[:,1],linewidth=linewidth1,color=line_colors2)
    #axb.plot(k,Ek[:,2],linewidth=linewidth1,linestyle='--',color=line_colors3)
    #axb.plot(k,Ek[:,3],linewidth=linewidth1,linestyle='--',color=line_colors4)
    # axb.scatter(k_mu05,Ek_mu05[:,0],linewidth=linewidth1,color=line_colors1_mu05,s=pointscale)
    # axb.scatter(k_mu05,Ek_mu05[:,1],linewidth=linewidth1,color=line_colors2_mu05,s=pointscale)
    # axb.scatter(k_mu05,Ek_mu05[:,2],linewidth=linewidth1,color=line_colors3_mu05,s=pointscale)
    # axb.scatter(k_mu05,Ek_mu05[:,3],linewidth=linewidth1,color=line_colors4_mu05,s=pointscale)
    for i in range(len(k)-1):
        axb.plot(k[i:i+2],qp_ek[i:i+2,0],color=line_colors1_mu05[i,:],lw=lw_band)
        axb.plot(k[i:i+2],qp_ek[i:i+2,1],color=line_colors2_mu05[i,:],lw=lw_band)
        axb.plot(k[i:i+2],qp_ek[i:i+2,2],color=line_colors3_mu05[i,:],lw=lw_band)
        axb.plot(k[i:i+2],qp_ek[i:i+2,3],color=line_colors4_mu05[i,:],lw=lw_band)
    axb.plot(k,len(k)*[0],linewidth=1,linestyle='--',color='C1')
    axb.set_box_aspect(1)
    # axb.set_ylabel(r'Energy',fontsize = legftsz,labelpad=6)
    axb.set_ylabel(r'$E_k/(2t)$',fontsize = legftsz, labelpad=-5)
    axb.set_xticks(tick_label)
    axb.set_xticklabels(name_label,fontsize = legftsz)
    # axb.set_xlabel('high symmetry points')
    axb.set_yticks([-4,-2,0,2,4])
    ...
    # axb.text(0.05, 0.95, r'C#=0', transform=axb.transAxes, fontsize=ftsz, va='top')
    # axb.text(x_labeltext, y_labeltext, '(b)', transform=axb.transAxes, fontsize=ftsz, va='top') # subfig label
    ...
    # axc.text(x_labeltext, y_labeltext, '(c)', transform=axc.transAxes, fontsize=ftsz, va='top') # subfig label
    ...
    # axd.text(0.05, 0.95, r'C#=0', transform=axd.transAxes, fontsize=ftsz, va='top')
    # axd.text(x_labeltext, y_labeltext, '(d)', transform=axd.transAxes, fontsize=ftsz, va='top') # subfig label
    ...
    # axe.text(x_labeltext, y_labeltext, '(e)', transform=axe.transAxes, fontsize=ftsz, va='top') # subfig label
    ...
    # cb = plt.colorbar(im, cax=cbar_ax, ticks=[-5,0,5],drawedges=False)
